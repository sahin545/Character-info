...
{Fore.YELLOW}║      Raihan-v FREE FIRE BOT         ║{Fore.RESET}
{Fore.YELLOW}║    Join @morpheusBlackEra (Tg)   ║{Fore.RESET}
{Fore.CYAN}╚════════════════════════════════════╝{Fore.RESET}
    """
    for char in banner:
...
def check_banned_status(player_id):
    url = f"https://amin-team-api.vercel.app/check_banned?player_id={player_id}"
    try:
        ...
        return {
            ...
            "is_banned": data.get("status") == "BANNED"  # True/False helper
        }
...
def  generate_random_word():
    word_list = [
  "Subscribe to Spideerio Gaming","Join @CraftLandEraGroup at Telegram"
    ]
...
class FF_CLIENT(threading.Thread):
    ...
    def GenResponsMsg(self, Msg):
        ...
        bot_uid = self.player_uid if self.player_uid else 13527374106
        clan_id = self.clan_id if self.clan_id else 3086685882
        ...
        2: {
            ...
            10: "en",
            13: {
            1: "https://yt3.googleusercontent.com/GZeoIYPVe-2Y43Po1xuFxMHWtMu35osmXsrv3iWZ50BQtu7qLIjMe8x7OOVKQArstiM7fbqm=s120-c-k-c0x00ffffff-no-rj",
            2: 1,
            3: 1
            },
            14: {}
    ...
    def joinclanchat(self):
        # Use auto-extracted values or fallback to defaults
        clan_id = self.clan_id if self.clan_id else 3086685882
        clan_key = self.clan_key if self.clan_key else "Wcb/AsDd39ViBu/Rm6oHdg"
                ...
                if "8" in parsed_data["5"]["data"] and "data" in parsed_data["5"]["data"]["8"]:
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"""[C][B]
Welcome to [00ff00]SPIDEERIO-AI-ERA[ffffff]!
...
####################################
            # ========= NEW AND MODIFIED MMANDS START HERE =========
###################################
           ...
           # ** NEW /info COMMAND **
            if "1200" in data.hex()[0:4] and b"/info" in data:
                try:
                    ...
                    # API Call
                    url = (
    f"https://lord-morpheus-main-morpheus-projects-a51f0a58.vercel.app/like"
    f"?uid={uid}&server_name={server2}"
    f"&x-vercel-set-bypass-cookie=true&x-vercel-protection-bypass={BYPASS_TOKEN}"
)
                    print(url)
                    response = requests.get(url)
                    print(response)
...
def run_client(id, password):
    ...
    loading("Initializing Spideerio's Free Fire Bot", duration=3)
